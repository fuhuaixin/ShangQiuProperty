...
public class AutoHeightViewPager extends ViewPager {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        Log.d("AutoHeightViewPager", "height=" + height + "  pos=" + mCurPosition);
    ...
    public void updateHeight(int current) {
        ...
        if (indexList.size() > 0) {
            Log.d("AutoHeightViewPager", "updateHeight 1 current=" + current + "   mCurPosition=" + mCurPosition);
            saveIndexData();
...
//                LogUtils.d("AutoHeightViewPager", "updateHeight 2=" + height + "   pos=" + current);
    ...
    private void saveIndexData() {
        if (!indexList.get(mCurPosition)) {//没保存高度时，保存
            ...
            Log.d("AutoHeightViewPager", "saveIndexData 1=" + mHeight + "   mCurPosition=" + mCurPosition);
        }
    }
